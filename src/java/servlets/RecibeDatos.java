...
public class RecibeDatos extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String empleo = request.getParameter("cargo"); 
        String iden = request.getParameter("identificacion"); 
        
        // Realizar cualquier operación con los datos aquí
        // Por ejemplo, guardar los datos en una base de datos
        
        // Pasar los resultados a una página JSP para mostrar visualmente
        request.setAttribute("nombre", nombre);
        request.setAttribute("apellido", ape);
        request.setAttribute("correo", mail);
        request.setAttribute("cargo", empleo);
        request.setAttribute("identificacion", iden);
        request.getRequestDispatcher("/registroResultado.jsp").forward(request, response);
